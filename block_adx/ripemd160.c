...
static const unsigned int initial_h[5] = { 0x67452301u, 0xEFCDAB89u, 0x98BADCFEu, 0x10325476u, 0xC3D2E1F0u };
...
/* Left line */
static const unsigned char RL[5][16] = {
...
/* Right line */
static const unsigned char RR[5][16] = {
...
/* Shifts, left line */
static const unsigned char SL[5][16] = {
...
/* Shifts, right line */
static const unsigned char SR[5][16] = {
...
static const unsigned int KL[5] = {
...
static const unsigned int KR[5] = {
...
void ripemd160_init(ripemd160_state *self)
{
	...
	memcpy_c(self->h, initial_h, RIPEMD160_DIGEST_SIZE);
	memset_c(&self->buf, 0, sizeof(self->buf));
	self->length = 0;
	self->bufpos = 0;
}

void byteswap32(unsigned int *v)
{
	union { unsigned int w; unsigned char b[4]; } x, y;
...
void byteswap_digest(unsigned int *p)
{
	unsigned int i;
...
{
	unsigned char w, round;
	unsigned int T;
	unsigned int AL, BL, CL, DL, EL;    /* left line */
	unsigned int AR, BR, CR, DR, ER;    /* right line */
	...
	while (length > 0) {
		...
		if ((unsigned long)length >= bytes_needed) {
			...
			* the compression function. */
			memcpy_c(&self->buf.b[self->bufpos], p, bytes_needed);
		...
		* Copy what's there and return. */
		memcpy_c(&self->buf.b[self->bufpos], p, length);
		self->bufpos += length;
		self->length += length*8;    /* length is in bits */
		return;
	}
}
...
void ripemd160_done(ripemd160_state *self, unsigned char *out)
{
	...
	/* Append the length */
	self->buf.w[14] = (unsigned int)(self->length & 0xFFFFffffu);
	self->buf.w[15] = (unsigned int)((shr64(self->length ,32)) & 0xFFFFffffu);
#ifdef PCT_BIG_ENDIAN
...
#endif
	memcpy_c(out, &self->h, RIPEMD160_DIGEST_SIZE);
}

OS_API_C_FUNC(int) ripemd160(const void* in, unsigned long length, void* out)
{
	ripemd160_state md;
	ripemd160_init(&md);
