...
C_IMPORT int C_API_FUNC read_objfile(mem_zone_ref_ptr p1, mem_zone_ref_ptr p2);


C_IMPORT int C_API_FUNC node_load_parse_tree(mem_zone_ref_ptr hash, mem_zone_ref_ptr out);
...
C_IMPORT int C_API_FUNC	node_get_service_desc(mem_zone_ref_ptr service_name, mem_zone_ref_ptr service_json);
C_IMPORT int C_API_FUNC	node_load_bookmark();


C_IMPORT int C_API_FUNC	tracer_new_scene(mem_zone_ref_ptr in);
C_IMPORT int C_API_FUNC	tracer_load_cubemap(mem_zone_ref_ptr in, mem_zone_ref_ptr in2);
C_IMPORT int C_API_FUNC	tracer_load_model(mem_zone_ref_ptr in, mem_zone_ref_ptr in2);

C_IMPORT int C_API_FUNC	 read_wave_file(mem_zone_ref_const_ptr wavefile, mem_zone_ref_ptr sample);
...
C_IMPORT int C_API_FUNC	node_disconnect(mem_zone_ref_ptr node);
C_IMPORT int C_API_FUNC	node_find_peer(mem_zone_ref_ptr node);
...
int main(int argc, const char **argv)
{
	...
	load_module				("modz/libbase.tpo", "libbase", &libbase_mod,1, PTR_NULL);
	...
	set_dbg_ptr2(node_add_block_header, accept_block, node_compute_pow_diff_after, store_block, PTR_NULL, node_get_script_modules, get_pow_reward, node_get_script_msg_handlers, node_get_mem_pool, node_del_txs_from_mempool, node_add_tx_to_mempool, store_wallet_tx, store_wallet_txs, queue_mempool_message, node_list_accounts, node_list_addrs, get_sess_account, node_has_service_module, queue_getblock_hdrs_message, node_get_root_app, node_get_root_app_addr, node_get_mempool_hashes, node_get_root_app_fee, node_get_apps, node_get_app, node_get_app_types_def, node_get_types_def, node_get_app_objs, node_add_tmp_file, check_tx_files, get_scene_list, get_nscene, node_del_btree_from_mempool, blog_get_ncats, blog_get_cats, blog_get_cat_posts, blog_get_cat, blog_get_post, blog_get_last_posts, node_broadcast_addr, node_add_remote_peers, node_set_bookmark, node_load_bookmark, blog_get_accounts, blog_get_msgs, read_objfile, tracer_new_scene, tracer_load_cubemap, tracer_load_model, node_get_service_desc, read_wave_file, node_load_parse_tree, node_disconnect, node_find_peer);
