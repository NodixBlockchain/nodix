...
OS_API_C_FUNC(XML_Parser) XML_ParserCreate(const XML_Char *encodingName)
{
  ...
  memset_c(&position, 0, sizeof(POSITION));
...
OS_API_C_FUNC(void) XML_ParserFree(XML_Parser parser)
{
  for (;;) {
    TAG *p;
    if (tagStack == 0) {
...
OS_API_C_FUNC(void) XML_SetUserData(XML_Parser parser, void *p)
{
...
OS_API_C_FUNC(void) XML_SetElementHandler(XML_Parser parser,
...
OS_API_C_FUNC(void) XML_SetCharacterDataHandler(XML_Parser parser,
...
OS_API_C_FUNC(int) XML_Parse(XML_Parser parser, const char *s, int len, int isFinal)
{
  if (len == 0) {
    if (!isFinal)
      return 1;
